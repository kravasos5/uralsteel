...
async def get_brands(uow: UOWDep, offset: int = 0, limit: int = 100):
    """Получение марок стали"""
    brandsteel = await BrandSteelService().retrieve_all(uow, offset, limit)
    return brandsteel


@router.post('/create', response_model=BrandSteelReadDTO)
async def create_brandsteel(uow: UOWDep, brandsteel_data: BrandSteelCreateUpdateDTO):
    """Создание марки стали"""
    new_brandsteel = await BrandSteelService().create_one(uow, brandsteel_data)
    return new_brandsteel


@router.get('/{object_id}', response_model=BrandSteelReadDTO)
async def get_brandsteel(uow: UOWDep, object_id: GetIdDEP):
    """Получение марки стали"""
    brandsteel = await BrandSteelService().retrieve_one(uow, id=object_id)
    await error_raiser_if_none(brandsteel, error_raiser_message)
    return brandsteel


@router.put('/update/{object_id}', response_model=BrandSteelReadDTO)
async def update_brandsteel_put(
        uow: UOWDep,
        object_id: GetIdDEP,
        brandsteel_data: BrandSteelCreateUpdateDTO
):
    """Обновление марки стали методом put"""
    updated_brandsteel = await BrandSteelService().update_one(uow, brandsteel_data, id=object_id)
    await error_raiser_if_none(updated_brandsteel, error_raiser_message)
    return updated_brandsteel


@router.patch('/update/{object_id}', response_model=BrandSteelReadDTO)
async def update_brandsteel_patch(
        uow: UOWDep,
        object_id: GetIdDEP,
...
async def delete_brandsteel(uow: UOWDep, object_id: GetIdDEP):
    """Удаление марки стали по id"""
    deleted_brandsteel = await BrandSteelService().delete_one(uow, id=object_id)
    await error_raiser_if_none(deleted_brandsteel, error_raiser_message)
