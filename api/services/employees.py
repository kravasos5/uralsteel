...
class EmployeesService(ServiceBase):
    ...
    def retrieve_one_by_id(self, uow: AbstractUnitOfWork, user_id: int):
        """Получение пользователя по id"""
        with uow:
            employee = uow.repositories[self.repository].retrieve_one(id=user_id)
            return employee

    def retrieve_one_by_slug(self, uow: AbstractUnitOfWork, user_slug: str):
        """Получение пользователя по slug"""
        with uow:
            employee = uow.repositories[self.repository].retrieve_one(slug=user_slug)
            return employee
    ...
    def create_user(self, uow: AbstractUnitOfWork, data: EmployeesCreateDTO):
        ...
        with uow:
            ...
            new_employee = uow.repositories[self.repository].create_one(data=data)
            uow.commit()
            return new_employee
